...
bool seek_exact(terms_enum_t *terms_enum, bytes_ref_t *target)
{
    ...
    if (terms_enum->current_frame != &terms_enum->static_frame) {
        ...
        target_up_to = 0;
        terms_enum->current_frame = push_frame_bytes_ref(terms_enum, bytes_ref_add(output, arc->next_final_output), 0);
    }

    while (target_up_to < target->length) {
        uint32_t target_label = (uint32_t)(target->bytes[target->offset + target_up_to] & 0xFF);

        arc_t *next_arc = find_target_arc(
            &terms_enum->field_reader->index, target_label, arc, get_arc(terms_enum, 1 + target_up_to), &terms_enum->fst_reader);
        ...
        if (next_arc == NULL) {
            ...
            if (arc_is_final(arc)) {
                terms_enum->current_frame
                    = push_frame_bytes_ref(terms_enum, bytes_ref_add(output, arc->next_final_output), target_up_to);
            }
        }
    }
...
static terms_enum_frame_t *push_frame_bytes_ref(terms_enum_t *terms_enum, bytes_ref_t *frame_data, uint32_t length)
{
    wram_reader_t reader = { .buffer = frame_data->bytes, .index = frame_data->offset };
