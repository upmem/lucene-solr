...
uint8_t wram_read_byte(wram_reader_t *reader) { return reader->buffer[reader->index++]; }
...
void wram_skip_bytes(wram_reader_t *reader, uint32_t length) { reader->index += length; }
...
uint32_t wram_read_false_vlong(wram_reader_t *reader) { return wram_read_vint(reader); }