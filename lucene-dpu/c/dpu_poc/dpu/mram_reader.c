...
void set_index(mram_reader_t *input, uint32_t index) { input->index = input->base + index; }
...
{
    // todo something more efficient?
    return ((((uint32_t)mram_read_short(reader, decrement)) & 0xffff) << 16)
        | (((uint32_t)mram_read_short(reader, decrement)) & 0xffff);
}
...
uint32_t mram_read_false_vlong(mram_reader_t *reader, bool decrement) { return mram_read_vint(reader, decrement); }
