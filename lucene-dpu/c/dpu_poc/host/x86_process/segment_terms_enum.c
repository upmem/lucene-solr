...
static segment_terms_enum_frame_t *
push_frame_bytes_ref(segment_terms_enum_t *terms_enum, arc_t *arc, bytes_ref_t *frame_data, uint32_t length);

static segment_terms_enum_frame_t *
push_frame_fp(segment_terms_enum_t *terms_enum, arc_t *arc, uint64_t fp, uint32_t length);
...
static void
segment_term_enum_frame_init(segment_terms_enum_frame_t *frame, segment_terms_enum_t *term_enum, int32_t ord);
...
static void
decode_term(int64_t *longs, data_input_t *in, field_info_t *field_info, block_term_state_t *state, bool absolute);
...
static void
read_vint_block(data_input_t *doc_in, int32_t *doc_buffer, int32_t *freq_buffer, uint32_t num, bool index_has_freq);

static void
decode(packed_int_decoder_t *decoder, uint8_t *blocks, uint32_t blocks_offset, int32_t *values, uint32_t values_offset,
       uint32_t iterations);
...
static seek_status_t scan_to_term(segment_terms_enum_frame_t *frame, bytes_ref_t *target, bool exact_only) {
    return frame->is_leaf_block ? scan_to_term_leaf(frame, target, exact_only) : scan_to_term_non_leaf(frame, target,
                                                                                                       exact_only);
}
...
decode_term(int64_t *longs, data_input_t *in, field_info_t *field_info, block_term_state_t *state, bool absolute) {
    ...
    bool field_has_offsets =
            compare_index_options(field_info->index_options, INDEX_OPTIONS_DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >=
            0;
